...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/uwp-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/uwp-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && changedFileName !== 'Jenkinsfile' && changedFileName !== 'README.md' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../uwp-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/uwp-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../uwp-docs/` + changedFileName, `../../../gitlabRepo/uwp-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/uwp-docs/')) {
                            shelljs.cp('-rf', `../uwp-docs/` + changedFileName, `../../../gitlabRepo/uwp-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/uwp-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/uwp-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/uwp-docs`);
